...
Snake.controller('gameCtrl', function ($scope, $window, $interval, $http) {
  ...
  $scope.keyPress = function ($event) {
    ...
    if ($event.keyCode === 87 || $event.keyCode === 38) {         // if the key is w, for up
      newDir = 1;
    } else if ($event.keyCode === 68 || $event.keyCode === 39) {  // key is d for right
      newDir = 2;
    } else if ($event.keyCode === 83 || $event.keyCode === 40) {  // key is s for down
      newDir = -1;
    } else if ($event.keyCode === 65 || $event.keyCode === 37) {   // key is a for left
      newDir = -2;
    } else if ($event.keyCode === 75) {  // key is k
    ...
    } else if ($event.keyCode === 74) {  // key is j to change player
  ...
  $scope.startGame = function () {
    ...
    // set game speed to the user selected level
    $scope.gameState.speed = Number($scope.level);
  ...
  var checkFrame = function (gameState, $scope, $interval) {
    ...
    } else {  // no collisions, update display
  ...
  var updateGameBoard = function ($scope) {
    $scope.canvasCtx.fillStyle = 'black';
    ...
    $scope.canvasCtx.fillStyle = '#FF4500';
